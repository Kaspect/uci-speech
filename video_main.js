...
var constraints = { audio: {
        ...
        sampleRate: 96000   // Disables noise suppression
    }, video: { width: { min: 640, ideal: 1024, max: 1024 }, height: { min: 480, ideal: 480, max: 2048 }, framerate: 60 } };
...
var video_start_unixtime_ms = "NOT STARTED YET";
...
function onBtnRecordClicked() {
    if (localStream == null) {
        ...
        mediaRecorder.onstart = function() {
            ...
            video_start_unixtime_ms = Date.now().toString()
             log('Start Time in Unixtime_ms = ' + video_start_unixtime_ms);
            ...
            localStream.getTracks().forEach(function(track) {
                ...
                if (track.kind == "video") {
                    log("onstart - Video track.readyState=" + track.readyState + ", track.muted=" + track.muted);
						...
						// start updating the soundmeter bar
            updateProgressBar();
        ...
        mediaRecorder.onstop = function() {
            ...
            var endtime = Date.now().toString()
            downloadLink.href = URL.createObjectURL(recording);
            log('End Time in Unixtime_ms = ' + endtime);

            switch (containerType) {
                case "video/mp4":
                    var name = "video_unixtime_ms_start_" + video_start_unixtime_ms + "_end_" + endtime + ".mp4";
                    break;
                default:
                    var name = "video_unixtime_ms_start_" + video_start_unixtime_ms + "_end_" + endtime + ".webm";
            }
...
SoundMeter.prototype.stop = function() {
    ...
    this.script.disconnect();
        ...
        // Assuming soundMeter is an object you have defined elsewhere
        // and soundMeter.instant is a value between 0 and 1

        function updateProgressBar() {
            var progressBarFilled = document.getElementById('progressBarFilled');
            var instantValue = soundMeter.slow; // Value between 0 and 1

            // Convert instantValue to a percentage
            var percentage = instantValue * 100;
            progressBarFilled.style.width = percentage + '%';
            progressBarFilled.textContent = percentage.toFixed(0) + '%';

            // Update the progress bar (ms)
